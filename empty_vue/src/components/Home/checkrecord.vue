  ...
  <div class="complete">
    <div class="my_complete_nav">
      <el-breadcrumb separator-class="el-icon-arrow-right" style="font-size: 18px;">
        ...
        <el-breadcrumb-item>档案复核</el-breadcrumb-item>
      </el-breadcrumb>
          ...
          <div class="my_one">
            ...
            <div class="my_one_right">
              <div style="width: 200px;height: 220px;background-color: rgb(243,243,243);border: 1px solid black;margin: 40px auto">
                <img v-if="id" :src="'http://47.112.132.177:8093/user/showimg/{id}?id='+id" class="avatar" style="height: 220px;width: 100%">
              </div>
              <el-upload
...
<script>
  ...
  export default {
    ...
    methods:{
      /*获取对应对象数据*/
      reqinfo(){
        var user = store.get('checklistdetail')
        this.id = store.get('listdetailid')
        console.log(user)
        var self = this.uerInfos
        self.userDeptIdOne = user.recordOne
        self.userDeptIdTwo = user.recordTwo
        self.userDeptId = user.recordThree
        self.userDeptSpecial = user.mpUser.userDeptSpecial
        ...
        self.userJobTitleId  = user.mpUser.userJobTitleId
        ...
        // self.userJobType = user.mpUser.recordOne
        // self.userJobName = user.mpUser.recordOne
        // self.userBirthDate = user.mpUser.recordOne
        // self.userStudy = user.mpUser.recordOne
      },
      /*三级机构变化相应的数据改变*/
      updateAll(){
        this.reqJobSpecial()
      ...
      /*复核通过*/
      async pass(){
        var result = await UPDATERECORD((store.get('checklistdetail')).id,0)
        if(result.status == 200){
          this.$message({
            type:'success',
            message:'复核通过成功，请重试',
