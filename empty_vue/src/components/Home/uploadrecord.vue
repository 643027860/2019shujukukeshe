    ...
    <div class="complete">
      <div class="my_complete_nav">
        <el-breadcrumb separator-class="el-icon-arrow-right" style="font-size: 18px;">
          ...
          <el-breadcrumb-item>档案上传</el-breadcrumb-item>
        </el-breadcrumb>
        <el-button type="primary" plain @click="uploadInfos" style="position: absolute;height: 35px;padding: 5px 20px;border-radius: 4px;margin-top: -10px;margin-left: 90%">上传</el-button>
      </div>
            ...
            <div class="my_one">
              <div class="my_one_left">
                ...
                <div  class="ge">
                  ...
                  <div class="ge_input">
                    <div style="width: 90%;display: inline-block;margin: 10px 5%">
                      <el-input v-model="uerInfos.userPhone" placeholder="如无则填写手机号"/>
                    </div>
                  </div>
                </div>
                <div  class="ge">
              ...
              <div class="my_one_right">
                <div style="width: 200px;height: 220px;background-color: rgb(243,243,243);border: 1px solid black;margin: 40px auto">
                  <img v-if="uerInfos.fileUrl" :src="uerInfos.fileUrl" class="avatar" style="height: 220px;width: 100%">
                </div>
                <el-upload
                  ...
                  :data="uerInfos">
                  <el-button type="primary" icon="el-icon-upload2" style="margin-left: 36px;height: 40px;font-size: 16px" v-if="!uerInfos.fileUrl">上传图片</el-button>
...
<script>
  ...
  import {REQSALARYLISTS,UPLOADUSER,REGONE,REQHOBBY,REQJOBTYPE,REQSPECIAL,REQJOBS,REQCHENG,REQEDU,REQMAJORJOB,UPLOADRECORD} from '../../api/types'
  export default {
    data(){
      return {
        userDeptIdOne: [],
    ...
    methods:{
      ...
      async uploadInfos(){
        ...
        &&userSalary&&userSex&&userSocialNo.trim()&&userZip.trim()&&userCreateDate&&userBirthDate){
          ...
          console.log(id.id)
          if(result.status == 200){
            var result1 = await UPLOADRECORD(user.userDeptIdOne,0,user.userDeptId,user.userDeptIdTwo,id.id)
